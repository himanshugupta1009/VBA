        ...
        for step in range(sep.max_steps):
            ...
            if episode_data is not None:
                dataset_collector.record_step(
                    ...
                    particle_states=par_states,
                    particle_weights=normalized_weights.detach().cpu().numpy(),
                    action=action,
                    reward=reward,
                    done=env.done,
...
def vts_lightdark_driver(shared_enc=False, independent_enc=False, load_paths=None, pre_training=True, save_pretrained_model=True,
    ...
    if dataset_dir is not None:
        dataset_config = {
            ...
            "image_format": "png",
            "image_resolution": "stored as returned by env.get_observation pre-normalization image",
            "fields_per_step": [
                "image_file",
                "state",
                "orientation",
